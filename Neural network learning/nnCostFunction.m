...
for i = 1: m
  %get current example with bias unit in the first layer  
  a1 = [1, X(i,:)];
  ...
  % actual output is 1, everything else - 0  
  tempY = (1:num_labels)' == y(i);
 
  %cost function with regularization
  J += - (sum(tempY.* log(h) + (1 - tempY).* log(1 - h)))/m;
  ...
  % scaling gradients(cost function is already scaled)
  Theta1_grad /=  m;
  Theta2_grad /=  m;
